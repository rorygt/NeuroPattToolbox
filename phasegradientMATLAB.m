...
if n > 1
   g(1,:) = anglesubtract(f(2,:), f(1,:), angleFlag)/(h(2)-h(1));
   g(n,:) = anglesubtract(f(n,:), f(n-1,:), angleFlag)/(h(end)-h(end-1));
end

% Take centered differences on interior points
if n > 2
   g(2:n-1,:) = anglesubtract(f(3:n,:), f(1:n-2,:), angleFlag) ./ (h(3:n) - h(1:n-2));
end
...
if ndim == 2
    ...
    if n > 2
        h = h(3:n) - h(1:n-2);
        g(:,2:n-1) = anglesubtract(f(:,3:n), f(:,1:n-2), angleFlag) ./ h;
    end
    varargout{2} = g;
...
elseif ndim > 2
    ...
    for k = 2:ndim
        ...
        % Take forward differences on left and right edges
        if n > 1
            g(:,1,:) = anglesubtract(nf(:,2,:), nf(:,1,:), angleFlag)/(h(2)-h(1));
            g(:,n,:) = anglesubtract(nf(:,n,:), nf(:,n-1,:), angleFlag)/(h(end)-h(end-1));
        end
        
        % Take centered differences on interior points
        if n > 2
            h = h(3:n) - h(1:n-2);
            g(:,2:n-1,:) = anglesubtract(nf(:,3:n,:), nf(:,1:n-2,:), angleFlag) ./ h;
        end
